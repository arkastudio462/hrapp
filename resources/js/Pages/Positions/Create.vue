...
const submit = () => form.post('/positions');
</script>
...
<template>
    ...
    <Link href="/positions" class="inline-flex items-center gap-1 text-sm font-semibold text-on-surface-variant transition-colors hover:text-on-surface">
        ...
        <div class="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8">
            <h1 class="font-display text-2xl font-bold tracking-tight text-ink">Tambah Jabatan</h1>
                ...
                <div class="rounded-2xl border border-slate-100 shadow-sm bg-white p-6">
                    <div class="space-y-4">
                        <div>
                            ...
                            <input v-model="form.name" type="text" required class="mt-1.5 block w-full rounded-full border border-stone-300 bg-white px-5 py-3 text-ink outline-none focus:border-sky-600" placeholder="Contoh: Staff Marketing" />
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input v-model="form.salary_grade" type="text" class="mt-1.5 block w-full rounded-full border border-stone-300 bg-white px-5 py-3 text-ink outline-none focus:border-sky-600" placeholder="Contoh: B1" />
                        </div>
                    </div>
                </div>
                <button
