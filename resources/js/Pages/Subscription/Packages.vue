...
<template>
    ...
    <AppLayout title="Paket Langganan">
    <Link href="/subscription" class="inline-flex items-center gap-1 text-sm font-semibold text-on-surface-variant transition-colors hover:text-on-surface">
        ...
        Kembali
            ...
            <div class="mt-10 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
                <div
                    ...
                    :key="pkg.id"
                    class="relative rounded-2xl border border-slate-100 shadow-sm bg-white p-6 transition-shadow hover:shadow-lg"
                >
                    <div v-if="pkg.is_popular" class="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-sky-600 px-3 py-1 text-xs font-bold text-white">
                        POPULER
                    </div>
